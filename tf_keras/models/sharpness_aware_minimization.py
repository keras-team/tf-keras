...
class SharpnessAwareMinimization(Model):
  ...
  Args:
    ...
    num_batch_splits: int. The number of mini batches to split into from each
      data batch. If None, batches are not split into sub-batches. Defaults to
      `None`.
  ...
  def train_step(self, data):
    ...
    Args:
      data: A nested structure of `Tensor`s. It should be of structure (x, y,
        sample_weight) or (x, y).
    ...
    if self.num_batch_splits is not None:
      ...
      if sample_weight is not None:
        sample_weight_split = tf.split(sample_weight, self.num_batch_splits)
      else:
