...
class SpectralNormalization(Wrapper):
    ...
    def _update_weights(self):
        ...
        kernel_weights, vector_u = tf.cond(
            ...
            lambda: self.normalize_weights(weights, vector_u),
        )
        self.kernel.assign(kernel_weights)
        self.vector_u.assign(vector_u)

    def normalize_weights(self, weights, vector_u):
        """Generate spectral normalized weights.

        This method will update the value of `self.kernel` with the
        spectral normalized value, so that the layer is ready for `call()`.
        """
        # Initialize vector_v to vector_u to help TensorFlow optimize the
        # computation.
        vector_v = vector_u
        vector_u = vector_u
